...
from resolution_engine import Literal, Clause, resolution
...
class ResolutionPreprocessor:
    def __init__(self):
        ...
        self.cnf_converter = CNFConverterLocal()
    ...
    def to_snf(self):
        """
        Преобразование в SNF:
        1. Сколемизация (удаление ∃)
        2. Сохраняем ∀ для последующего анализа
        """
        print("\nПРЕОБРАЗОВАНИЕ В SNF (удаление ∃ через сколемизацию)")
        self.expression = self.cnf_converter.skolemize(deepcopy(self.expression))
        self.print_analysis()
        return self.expression

    def remove_universal_quantifiers(self):
        """Удаляет все кванторы ∀"""
        def recursive_remove(expr):
            if isinstance(expr, LogicalExpression):
                expr.quantifiers = []
                if expr.left:
                    recursive_remove(expr.left)
                if expr.right:
                    recursive_remove(expr.right)
        recursive_remove(self.expression)
        print("\nВсе кванторы ∀ удалены:")
        self.print_analysis()

    def to_cnf(self):
        """Преобразует выражение в КНФ (после удаления ∀)"""
        print("\nПРЕОБРАЗОВАНИЕ В КНФ")
        self.expression = self.cnf_converter.convert(deepcopy(self.expression))
        self.print_analysis()
        return self.expression
...
def prepare_for_resolution(processor, clause_index):
    ...
    # 3. SNF (удаление ∃ через сколемизацию)
    print("\n3. ПРЕОБРАЗОВАНИЕ В SNF:")
    processor.to_snf()

    # 4. Удаляем ∀ (для построения КНФ)
    processor.remove_universal_quantifiers()

    # 5. КНФ
    processor.to_cnf()

    # 6. Стандартизация переменных
    print("\n6. СТАНДАРТИЗАЦИЯ ПЕРЕМЕННЫХ:")
